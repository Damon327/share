...
```bash
OLLAMA_HOST=127.0.0.1:6006 ollama run qwen3:4b   #2.5GB

#如果下载过慢，大家也可以下载较小的模型，比如：
OLLAMA_HOST=127.0.0.1:6006 ollama run qwen2.5:1.5b  #986MB

OLLAMA_HOST=127.0.0.1:6006 ollama run qwen2.5:0.5b  #398MB
```
...
![image-20251127200906561](images/image-20251127200906561.png)

### 5.2 XInference中下载大模型

#### 方式1：离线下载(推荐)

后续上传嵌入模型和重排序模型时，可能跟当前的XInference版本出现兼容性问题报错，原因是 pip 默认安装的 `xoscar v7.0+` 版本api与 `xinference` 不兼容，升级到`xoscar v0.8.0` 后可以正常启动。

**① 指定xoscar版本：**

```bash
pip install xoscar==0.8.0
...
![image-20251128001026771](images/image-20251128001026771.png)

2）使用下载的模型

huggingface平台下载：（还可以从ModelScope平台下载）

bge-large-zh-v1.5地址：https://huggingface.co/BAAI/bge-large-zh-v1.5/tree/main

点击下载：

![image-20251127220328376](images/image-20251127220328376.png)
...
```bash
root@autodl-container-ad3a418634-fcc8d57d:~# pwd    
/root
...
> 说明：这些是客户端命令，需要连接启动的xinference服务器，确保xinference已经启动！
>

xinference默认用的是9997端口号，我们教案启动命令：xinference-local --host 0.0.0.0 --port 6006，指定的端口号是6006，所以xinference 客户端命令都需要通过 `--endpoint` 参数明确告诉它们你的服务运行在哪个地址。

加载模型：

```bash
# 执行xinference launch，加载本地上传的模型
xinference launch \
...
> 注意： xinference 1.4.0 以后支持reranker！
>
> 在较旧的 XInference 版本中，模型类型只支持 embedding、llm 和 cross-encoder 等。reranker 是后来新增的模型类型。这里我们安装的1.13.0版本中有这个类型的定义。

1）使用现有资料的模型`（推荐）`

![image-20251128001107405](images/image-20251128001107405.png)
...
![image-20251127205003232](images/image-20251127205003232.png)
...
![image-20251127231129860](images/image-20251127231129860.png)
